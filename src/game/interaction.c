    ...
    if (m->action != ACT_GETTING_BLOWN && capFlag != 0) {
        ...
        switch (capFlag) {
            case MARIO_VANISH_CAP:
                capTime = 600 + SM64AP_GetProgressiveVanishCapBonusFrames();
                capMusic = SEQUENCE_ARGS(4, SEQ_EVENT_POWERUP);
                break;

            case MARIO_METAL_CAP:
                capTime = 600 + SM64AP_GetProgressiveMetalCapBonusFrames();
                capMusic = SEQUENCE_ARGS(4, SEQ_EVENT_METAL_CAP);
                break;

            case MARIO_WING_CAP:
                capTime = 1800 + SM64AP_GetProgressiveWingCapBonusFrames();
                capMusic = SEQUENCE_ARGS(4, SEQ_EVENT_POWERUP);
                break;
        }
